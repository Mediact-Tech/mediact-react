...
## เปิด drawer จากโค้ดของแอป (host bridge)

ปุ่ม/แบนเนอร์ที่อยู่ลึกในหน้า feature อยากเปิดแชท (พร้อมยิงคำถามให้เลย) โดยไม่ต้องลากสถานะ
ขึ้นไปถึง root layout — ใช้ `openAiChat()` ได้จากทุกที่:

```tsx
import { openAiChat } from "@mediact/react/ai-chat";

const handled = openAiChat({
  message: "ช่วยตรวจกฎของตารางเวรเดือนนี้ และแนะนำวิธีแก้จุดที่ติดกฎให้หน่อย",
  mode: "schedule", // สลับโหมดให้ก่อนส่ง (ไม่ใส่ = โหมดปัจจุบัน)
});
if (!handled) {
  // ไม่มี widget mount อยู่ (เช่น ยังไม่ล็อกอิน / ไม่ได้ตั้ง baseUrl) — ซ่อนปุ่มหรือแจ้งผู้ใช้เอง
}
```

พฤติกรรม:
- drawer เปิดทันที ข้อความ **รอจนต่อห้องเสร็จ** (หรือรอเทิร์นที่ค้างอยู่จบ) แล้วค่อยส่งเป็น turn ของผู้ใช้ตามปกติ
- `scope` ใช้ค่าเดียวกับที่ host ส่งเข้า widget ผ่าน props (อ่านใหม่ตอนส่งเสมอ) — ไม่ต้องแนบมากับ event
- เรียกโดยไม่ใส่ `message` = เปิด drawer เฉยๆ
- ถ้าต่อห้องไม่สำเร็จ ข้อความที่คิวไว้จะถูกทิ้ง (ไม่ยิงย้อนหลังตอนผู้ใช้กด retry เอง)
- กลไกข้างใต้คือ window `CustomEvent` (`AI_CHAT_OPEN_EVENT`) — widget ที่ mount อยู่ `preventDefault()`
  เพื่อบอกว่ารับเรื่องแล้ว ค่าที่ `openAiChat` คืนจึงเชื่อถือได้ว่ามีคนรับจริง
