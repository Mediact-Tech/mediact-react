...
describe("ด่านกันสีดิบ", () => {
  ...
  it("จำนวนสีดิบที่เหลือต้องไม่เพิ่ม", () => {
    ...
    /* **+1 = 132 (2026-08-18)** — `text-white` ของช่องเดือน/ปี **ที่เลือกอยู่** ใน `Calendar`
     * · เดิมเป็น `text-brand-foreground` ซึ่ง **Mediwork ตั้งทับเป็น `#151d27` เอง**
     *   (`styles/design-system.css:118`) ⇒ ช่องที่เลือกเป็นตัวหนังสือเกือบดำที่นั่น
     *   ขณะที่อีก 3 แอปเป็นขาว — ผู้ใช้เคาะจากจอจริงว่าต้องขาวทุกแอป
     * · ทำไมไม่ใช้ token: `--color-text-inverse` มีเฉพาะใน semantic layer ชุดใหม่
     *   **ไม่มีใน `theme.css` ที่ Mediwork โหลดจริง** ⇒ ใช้แล้วสีจะไม่เปลี่ยนที่แอปนั้นเลย
     *   (กับดักเดียวกับ tooltip/AppShowcaseDialog ที่บันทึกไว้ข้างบน)
     * ⇒ ลดกลับเป็น 131 ในวันที่ Mediwork ย้ายมากิน `@mediact/react/tokens.css` */
