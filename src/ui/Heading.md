...
> (Mediwork ได้มิ้นต์บนขาว 1.93:1 อ่านไม่ออก) · ตอนนี้ `default` ชี้ค่าเดียวกับ `heading`
...
| 2026-08-13 | ใส่ `m-0` ที่ base ของ `headingVariants` | `Heading` เรนเดอร์ `h1`–`h6` **จริง** ⇒ UA stylesheet ให้ margin ตามระดับ (`h2` = `0.83em` บนล่าง = **16.6px ที่ 20px** ซึ่งเป็นค่าที่วัดได้จาก Mediwork) · component ต้องไม่ขึ้นกับว่าแอปเปิด preflight หรือไม่ — เหตุผลเต็มใน [`Text.md`](Text.md) | เหมือน `Text` |
...
- `<Heading>` **ยังไม่ถูกใช้ใน DS เองเลยสักจุด** — จุดที่ใกล้ที่สุดคือ `DialogTitle` ซึ่งเรนเดอร์
  `<h2>` ของ Radix เองและตอนนี้ล้าง margin แยกไว้ (คลาสซ้ำกัน แต่ยังไม่ได้ยุบมาใช้ตัวเดียวกัน)
