...
| 2026-08-14 | **เพิ่ม hover เส้นขอบเป็นสีแบรนด์** (`enabled:hover:border-brand` ใน `fieldShapeClasses`) | 🔴 ปิดช่องว่างที่แถวบนจดไว้เอง — ทุก field ของ DS มีแต่ `focus` ⇒ เอาเมาส์ไปวางแล้วไม่มีอะไรตอบสนอง ซึ่งเป็นวิธีที่ทั้งหน้าเว็บใช้เขียนคำว่า *กดไม่ได้* · ของจริงที่ผู้ใช้ใช้อยู่ทุกวันมี: จอตั้งขอบเขตทั้ง 3 จอของ Mediwork (ตารางเวรพยาบาล · ตารางเวรแพทย์ · ภาพรวมอัตรากำลัง) ตั้ง `'&:hover fieldset': {borderColor: var(--primaryColor)}` ไว้ตรง ๆ (`components-v2/shared/selectionFieldSx.ts:27`) ⇒ ช่องของ DS ที่วางข้างกันดูตายกว่าอย่างเห็นได้ | ทุกแอปได้ hover พร้อมกันโดยไม่ได้ขอ — ตรวจแล้วว่าไม่มีแอปไหนมี hover ของตัวเองบน field ของ DS ให้ชน · `enabled:` กันช่องที่ปิดอยู่ไม่ให้สัญญาสิ่งที่ทำไม่ได้ · สถานะผิดพลาด**ไม่มี** hover โดยตั้งใจ (เส้นแดงคือข้อความ ไม่ใช่ของตกแต่ง) |
| 2026-08-14 | **มุมโค้งอ่านจาก `--radius-field` · ค่าสำรอง = ของเดิม 4px** | ของจริงในแอปไม่ตรงกัน: จอตั้งขอบเขตของ Mediwork ใช้ **8px** มาตลอด (`selectionFieldSx.ts:25`) ส่วน DS ใช้ 4px ⇒ ช่องของ DS ที่วางข้างของเดิมมุมคมกว่า · ⚠️ **ไม่เปลี่ยนค่าตั้งต้นให้ทุกแอป** เพราะกฎข้อ 1 ของ repo: ค่าที่ใช้ร่วม 4 แอปต้องวัดจากจอจริงของทั้ง 4 ก่อน — ตอนนี้มีตัวเลขจาก Mediwork แอปเดียว | มีตัวแปรเพิ่มมาอีกตัวที่ต้องรู้ว่ามีอยู่ · แอปที่ไม่ตั้งอะไรไม่เปลี่ยนสักพิกเซล · 🚪 **ยังไม่ยุบเป็นค่าเดียว** — ต้องวัด Portal/Medimatch/MediHR ก่อน |
